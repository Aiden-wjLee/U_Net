...
def main(): 
    ...
    net_test.load_state_dict(torch.load('/home/BEST/BEST35_save_256.pth'))
    #torch.jit.load(): 모델을 저장할 때 사용한 방식과 동일한 방식으로 모델을 불러올 수 있습니다.
    ...
    with torch.no_grad(): # validation 이기 때문에 backpropa 진행 x, 학습된 네트워크가 정답과 얼마나 가까운지 loss만 계산
        ...
        for batch, (inputs,label) in enumerate(val_gen): 
            ...
            cv2.imshow("in1",inputs.squeeze()[0])
            cv2.imshow("in2",inputs.squeeze()[0])
            inputs_numpy=inputs
            ...
            cv2.imshow("out_class1",1/2*output_numpy.squeeze()[0][1])
            cv2.imshow("out_class2",output_numpy.squeeze()[0][2])
            ...
            cv2.imshow("in+out(0)",in_out[0])
            cv2.imshow("in+out(1)",in_out[1])
